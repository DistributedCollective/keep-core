...
describe('TokenStakingEscrow', () => {
  ...
  describe('receiveApproval', async () => {
    it('reverts for unknown token', async () => {
      ...
      await anotherToken.approve(escrow.address, grantedAmount, { from: tokenStaking })
      await expectRevert(
        escrow.receiveApproval(tokenStaking, grantedAmount, anotherToken.address, data),
        "Not a KEEP token"
      )
    })
    ...
    //   await token.approve(escrow.address, grantedAmount, { from: tokenStaking })
    //   await expectRevert(
    //     escrow.receiveApproval(tokenStaking, grantedAmount, token.address, corruptedData, { from: tokenStaking }),
    ...
    //   await token.approve(escrow.address, grantedAmount, { from: tokenStaking })
    //   await expectRevert(
    //     escrow.receiveApproval(tokenStaking, grantedAmount, token.address, data),
    ...
    //   await token.approve(escrow.address, grantedAmount, { from: tokenStaking })
    //   await escrow.receiveApproval(tokenStaking, grantedAmount, token.address, data)
    ...
    //   await token.approve(escrow.address, grantedAmount, { from: operator })
    //   await expectRevert(
    //     escrow.receiveApproval(operator, grantedAmount, token.address, data),
    //     "Only owner can deposit"
    //   )

    //   await token.approve(escrow.address, grantedAmount, { from: grantee })
    //   await expectRevert(
    //     escrow.receiveApproval(grantee, grantedAmount, token.address, data),
    //     "Only owner can deposit"
    //   )

    //   await token.approve(escrow.address, grantedAmount, { from: deployer })
    //   await expectRevert(
    //     escrow.receiveApproval(deployer, grantedAmount, token.address, data),
    //     "Only owner can deposit"
    //   )
    // })
  })
  ...
  //     await token.approve(escrow.address, depositedAmount, { from: tokenStaking })
  //     await escrow.receiveApproval(tokenStaking, depositedAmount, token.address, data)
  //   })
  ...
  //     await token.approve(escrow.address, 600, { from: tokenStaking })
  //     await escrow.receiveApproval(tokenStaking, 600, token.address, data)
